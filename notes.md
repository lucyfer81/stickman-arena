...
## Round D — 美术总监发布审查（第三轮）：战斗舞台「深度重塑」

以美术总监视角做发布前全场景审查。本轮与前两轮（Round 8 标题 / Round 9 结算）的关键差异：用图像分析工具对
**当前 build**（Round 15 MERCY + Round C 之后）重抓全部关键帧逐场景复审。

**排查结论（逐场景）：**
- 标题屏（Round 8 重塑）：仍良好——构图干净、海报感强，无需再动。
- 结算屏（Round 9 重塑）：良好——聚光灯+滚动分数+留存钩子仍在，分层背景反而强化了战败氛围。
- Boss / Second Wind / 移动横屏：复核无回归。
- **最伤玩家感知的是战斗背景（玩家 95% 时间盯着的画面）。** 量化+定性证据：
  - **画面非空率仅 8–9%**（wave1/combat），是全游戏最稀疏的画面——比标题(18–20%)、结算(14–15%)都低。
  - **每一轮独立审查（旧帧+新帧）都点名同一个问题**：背景是**单层平贴剪影**、建筑**底部悬浮在地面上方 ~30px**
    （"floating buildings"业余 tell）、地面是**无质感的纯色块**——读起来像占位舞台，而非完工竞技场。
  - 14 轮手感/反馈打磨（顿帧/慢镜/粒子/缩放）放在这样一个空荡舞台上播放——像顶级拳手在纸板擂台比赛。
  - 这是发布时**第一眼廉价感**的最大来源，玩家每秒都看到。

**根因（源码核实，`js/utils/background.js`）：** 原背景只有一层天际线（`bg` depth -90），建筑基线
`skylineY = GROUND_Y - 30` 即悬浮于地面线上方 30px，露出天空缝隙；地面是一块 `0x070912` 纯色矩形 + 细网格，
无反射/无暗角/无第三深度层。

**修复（纯视觉，仅 `background.js` + `TitleScene.js` 一行；零机制/时序/伤害/遥测 shape 改动）：**
- **三层深度**（单一最大杠杆）：新增 **FAR 天际线**（depth -95，淡蓝灰 0x2b3c5c、低对比 = 大气透视的"远"）
  + **NEAR 天际线**（depth -85，近黑 0x05080f、高对比、强窗光 = "近"），基底直达 GROUND_Y（消除悬浮缝）。
  两层明度差让深度一眼可读。
- **地平线霾带**（depth -88）+ **前景薄雾面纱**（depth -84，在近层之前）：低 alpha 蓝色横带，分隔天/地、
  隐去接缝、 veil 近层建筑基底——"premium"质感的最大单一 tell。
- **霓虹地面线反射**（depth -58，ADD 混合）：霓虹边在抛光地面上的垂直衰减倒影——"打光舞台"而非虚空。
- **暖色余烬**（新增第二组粒子，0xffae5c）混入原有青色余烬——冷调中的颜色生气。
- **底部暗角**：压住地面、把视线收回到动作。
- 修了一个潜伏无限循环 bug：天际线随机宽度用 JS `%`（负被除数得负），曾产出负建筑宽度 → 死循环
  （新 `rand()` helper 保证非负）。
- **前景框（边缘立柱）试做后撤掉**：图像分析反复把它读成"扁平 UI 边框"而非深度（暗色板贴在屏幕边缘
  天然像窗口框），且在标题海报上显得拥挤——感知失败即弃用（保留 no-op 开关以便后续再调）。
- **标题屏传 `{skipForeground:true}`**：即使前景框被禁用也守卫标题的干净构图。

**设计取舍（多解取更享受的）：**
- 保留"暗剪影"美术语言（不引入写实贴图）——深度靠**明度差 + 雾 + 反射**，而非违背既有程序化风格。
- "subtle 失败"教训：第一版 haze(α≈0.1)/reflection(α≈0.16) 太淡，图像分析（玩家瞥一眼的代理）完全读不到；
  反复试探后把 haze 提到 α≈0.34、reflection 提到 α≈0.22 + ADD 混合、近层窗光提到 0.6+，才稳定可读。
  教训：**深度要素必须强到一眼可读，否则等于没做**。
- 前景框弃用而非强改：两次独立分析都读成 UI 边框 = 设计意图（深度）与感知（边框）不匹配，取感知优先。

**验证（本轮图像直读 + imgstat + 全测试）：**
- 图像分析逐项确认（wave1 帧）：①两层不同明度建筑（远淡近暗）✅ ②建筑接地无悬浮缝 ✅ ③地平线霾带 veil
  建筑基底 ✅ ④霓虹地面线下方蓝色反射 ✅ ⑤边缘干净无框 ✅。结论："layered, grounded, atmospheric arena
  rather than a flat placeholder"。
- 结算屏：分层背景"强化战败氛围，不与 UI 抢戏，全部文字清晰"。
- 移动横屏：分层深度在窄比例上仍可读；角色/UI 清晰。
- 标题屏：前景框 gating 后"exceptionally clean and well-designed, poster-like, no clutter"——Round 8 成果守住。
- 官方 CI **5/5 绿**（core 3 + 移动横/竖）。
- dev 关键套件全绿：firstminute 11/11（直接跑标题+早期游戏，验证新背景）、options 10/10、qa 7/7、feel14 1/1、
  depth 3/3、laststand 4/4、mercy 7/7、retention 5/5、variety/bossvariety 29/29 —— **零回归**。
- 新增 `tools/capture.cjs`：独立、稳健的艺术抓帧脚本（每个场景独立 try/catch + 遥测校验），替未来的美术复审。

**感知评估：** 战斗舞台从"单层平贴剪影 + 悬浮建筑 + 纯色地面 = 占位舞台"变成"远近双层天际线 + 地平线霾 +
霓虹反射的分层竞技场"——把玩家 95% 时间盯着的画面从最大廉价感来源变成质感基底。仍是图像代理指标
（无真人观感）；诚实保留：①Boss 帧的图像分析读得很苛刻（误报 Boss 体型同玩家、忽视既有 Round 14 手感），
但那是单帧分析局限而非实景问题；②前景深度层（边缘立柱）这条路被弃，若未来想要第三深度平面，应做成
